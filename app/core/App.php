...
class App
{
    ...
    public function __construct()
    {
        try {
            ...
            error_log("Unhandled Exception: $mess in $file on line $line");
            
            if (self::$app) {
                self::$app->loadError('database', ['message' => $mess, 'line' => $line, 'file' => $file]);
            } else {
                echo "Fatal Error: $mess";
            }
            die();
        }
    }
    ...
    public function loadError($nameError, $data = [])
    {
        ...
        // Detect if this is likely an API request
        $url = $_GET['url'] ?? '';
        $isApi = (strpos($url, 'Api') !== false || 
                  strpos($url, 'check') !== false || 
                  strpos($url, 'isIdenti') !== false ||
        ...
        if ($isApi) {
            ...
            echo json_encode([
                'error' => $message ?? 'Xảy ra lỗi hệ thống.',
                'success' => false,
                'code' => 500,
                'debug' => [
                    'type' => $nameError,
