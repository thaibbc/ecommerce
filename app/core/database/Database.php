...
class Database
{
    ...
    {
        global $dbConfig;
        error_log('Database constructor called, dbConfig: ' . (isset($dbConfig) ? json_encode($dbConfig) : 'not set'));
        $this->conn = Connection::getInstance($dbConfig);
        error_log('Database connection result: ' . (is_null($this->conn) ? 'null' : 'object'));
    }
    ...
    public function findAndUpdate($tableName, $dataToUpdate, $conditions = [])
    {
        ...
        try {
            ...
            $file = $e->getFile();
            $line = $e->getLine();
            error_log("Database Error (findAndUpdate): $mess in $file on line $line");
    ...
    public function findByIdAndUpdate($tableName, $id, $dataToUpdate)
    {
        try {
            ...
            $file = $e->getFile();
            $line = $e->getLine();
            error_log("Database Error (findByIdAndUpdate): $mess in $file on line $line");
            App::$app->loadError('database', ['message' => $mess, 'line' => $line, 'file' => $file]);
            die();
        }
    }
    ...
    public function create($tableName, $dataToInsert)
    {
        try {
            ...
            $file = $e->getFile();
            $line = $e->getLine();
            error_log("Database Error (create): $mess in $file on line $line");
            App::$app->loadError('database', ['message' => $mess, 'line' => $line, 'file' => $file]);
            die();
        }
    }
    ...
    function query($sql)
    {
        try {
            ...
            $file = $e->getFile();
            $line = $e->getLine();
            error_log("Database Error (query): $mess in $file on line $line");
            App::$app->loadError('database', ['message' => $mess, 'line' => $line, 'file' => $file]);
            die();
        }
    }
